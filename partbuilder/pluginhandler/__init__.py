...
class PluginHandler:
    ...
    def __init__(
        ...
        *,
        ...
        self._scriptlet_metadata: Dict[
            steps.Step, extractors.ExtractedMetadata
        ] = collections.defaultdict(extractors.ExtractedMetadata)
    ...
    def _set_version(self, *, version):
        try:
            self._set_scriptlet_metadata(
                extractors.ExtractedMetadata(version=version)
            )
    ...
    def _set_grade(self, *, grade):
        try:
            self._set_scriptlet_metadata(
                extractors.ExtractedMetadata(grade=grade)
            )
        except errors.ScriptletDuplicateDataError as e:
            raise errors.ScriptletDuplicateFieldError("grade", e.other_step)

    def _check_scriplet_metadata_dupe(
        self, metadata: extractors.ExtractedMetadata, step: steps.Step
    ):
    ...
    def _set_scriptlet_metadata(self, metadata: extractors.ExtractedMetadata):
        try:
            step = self.next_step()
    ...
    def _generate_part_env(self, step: steps.Step) -> str:
        ...
        # Snapcraft's say.
        snapcraft_build_environment = get_part_environment(self, step=step)
