...
class Validator:
    def __init__(self, parts=None):
        ...
        self._input = parts if parts else {}
        self._load_schema()
    ...
    def _load_schema(self):
        print(common.get_schemadir())
        schema_file = os.path.abspath(
            os.path.join(common.get_schemadir(), "parts.json")
        )
        try:
    ...
    def validate(self, *, source="snapcraft.yaml"):
        ...
        try:
            jsonschema.validate(
                self._input, self._schema, format_checker=format_check
            )
